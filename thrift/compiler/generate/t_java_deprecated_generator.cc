...
void t_java_deprecated_generator::generate_java_struct_writer(
  ...
  for (f_iter = fields.begin(); f_iter != fields.end(); ++f_iter) {
    ...
    bool optional = (*f_iter)->qualifier() == t_field_qualifier::optional;
    if (optional) {
...
void t_java_deprecated_generator::generate_java_struct_tostring(
  ...
  if (generate_immutable_structs_) {
    ...
    for (const t_field& f_iter : tstruct->fields()) {
      ...
      bool could_be_unset = field->qualifier() == t_field_qualifier::optional;
      if (could_be_unset) {
...
void t_java_deprecated_generator::generate_java_meta_data_map(
  ...
  for (const t_field& f_iter : tstruct->fields()) {
    ...
    } else if (field->qualifier() == t_field_qualifier::optional) {
