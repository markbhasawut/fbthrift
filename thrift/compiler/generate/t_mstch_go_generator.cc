...
class t_mstch_go_generator : public t_whisker_generator {
 ...
 private:
  ...
  bool is_field_inside_union(const t_field& field) const {
    // Whether field is part of a union. Union fields will always be indexed by
    // the base Whisker generator's context visitor. `parent` can only be
    // nullptr for throws/param list fields, and ephemeral request/response
    // structs which are instantiated by this generator.
  ...
  prototype<t_field>::ptr make_prototype_for_field(
    ...
    def.property("go_setter_name", [this](const t_field& self) {
      if (const auto& it = data_.field_setter_names.find(&self);
